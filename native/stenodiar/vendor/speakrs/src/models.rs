...
fn required_files(mode: ExecutionMode) -> Vec<String> {
    ...
    match mode {
        ExecutionMode::Cpu => {
            ...
            // stenograf patch: also fetch the batched/split/multi-mask ONNX
            // exports. With them present the CPU pipeline selects the
            // multi-mask embedding path (fbank once per chunk, ResNet trunk
            // shared across the per-speaker masks) and batched segmentation,
            // which measured ~2.7x faster end-to-end than the single-item
            // sessions the minimal file set forces.
