...
impl EmbeddingModel {
    ...
    pub(super) fn build_session_with_graph(
        ...
        // stenograf patch: pure-CPU embedding sessions were pinned to one
        // intra-op thread (parallelism upstream comes from CoreML/CUDA chunk
        // workers, which don't exist off those features), leaving CPU
        // diarization at ~1x realtime. Let ORT use the cores.
        let intra_threads = if mode == ExecutionMode::Cpu {
