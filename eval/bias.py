...
class Config:
    ...
    def is_baseline(self) -> bool:
        """``boost = 0`` disables biasing outright — the stock decode loop, the
        number every other config is measured against."""
        return self.alpha == 0
    ...
    def tag(self) -> str:
        if self.is_baseline:
            return "baseline"
        return f"a{self.alpha:g}-u{self.unk:g}-c{int(self.compound)}-n{self.n}"

    def __str__(self) -> str:
        if self.is_baseline:
            return "unbiased"
        tail = "" if self.compound else ", no compound-tail"
        return f"boost={self.alpha:g}, unk={self.unk:g}, N={self.n}{tail}"
...
def evaluate(
    ...
    *,
    ...
    for config in configs:
        ...
        hyps = decode(backend, config, audio, refs, tier=tier, seed=seed, force=force)
...
def gate(rows: list[Row]) -> bool:
    ...
    by_n = sorted(
        [r for r in biased if (r.config.alpha, r.config.unk, r.config.compound)
...
def run_distractor(backend_name: str, wavs: list[Path], *, terms: list[str], alpha: float) -> int:
    ...
    for wav in wavs:
        ...
        total += len(spans) + len(surface)
        verdict = "clean" if not spans and not surface else "CHANGED"
        print(f"{wav.name:<24} {verdict}: {len(spans)} edits, {len(surface)} surface")
        for a, b in spans[:5]:
            print(f"    {a or '∅'} → {b or '∅'}")
    ...
    biaser._restore()
    backend.unload()
    print(f"\nFALSE INSERTIONS: {total} — {'PASS' if total == 0 else 'FAIL'}")
...
def main() -> int:
    ...
    if args.tier == "distractor":
        ...
        if not pool_path.exists():
            raise SystemExit(
                f"missing {pool_path} — "
                "uv run --group eval eval/bias_data.py --fetch german"
            )
        pool = pool_path.read_text(encoding="utf-8").split()
        terms = random.Random(args.seed).sample(pool, args.distractors)
        return run_distractor(args.backend, wavs, terms=terms, alpha=args.boost)
