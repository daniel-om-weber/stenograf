...
(the local web UI). Phase 5 (Linux) is designed but deferred.** The per-task
...
summarized below, Stage C remains in full.
...
**Adopted recommendations still governing Stage C:** web server = **Starlette +
...
These are the interfaces Stage C's archive/reader/reverse-control views (C5–C7)
consume.
...
**Stage C — Web UI (`stenograf.web`). THE REMAINING PHASE 4 WORK.** The web view
is "a new `LiveView` + a `serve()` twin, zero core changes" — confirmed against
`view.py`/`tui.py`/`session.py`. Task sequence (independent, testable
increments; interface names illustrative; ``[dep:…]`` marks a hard
prerequisite):
...
**Ordering.** Stage C is all that remains: **C1 → C2 → C3 → C4**, with C5–C7
gating on the shipped Stage B interfaces and C8 packaging the assets. C8
re-touches packaging (the `web/static` assets), so re-run E4's clean-install
