...
Both devices pin at meeting start and a mid-meeting default switch is not
followed (unlike ``@DEFAULT_MONITOR@`` on Linux — WASAPI has no equivalent
alias); the mic pins to the configured device when there is one, else to the
default. No code path writes audio to disk.
"""
...
def default_devices(
    channels: set[Channel], *, mic_device: str | None = None
) -> dict[Channel, str]:
    ...
    choice is invisible in the device's own name. ``mic_device`` is the pinned
    microphone, validated by the same preflight.
    """
    for channel in sorted(channels):
        if channel is Channel.MIC and (blocked := mic_access_blocked()):
            raise CaptureUnavailableError(blocked)

    devices = query_devices(channels, mic_device=mic_device)
    return {
