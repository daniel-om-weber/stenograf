...
- **Windows**: two ``Stenograf.lnk`` shell links, in the Start Menu and on
  ...
  there is no console to flash. Where COM refuses, a Desktop
  ``Stenograf.cmd`` that ``start``\\ s the app is the degraded fallback.
...
def install_shortcut() -> Path | None:
    ...
    if sys.platform == "win32":
        return _install_windows_launcher()
    return None
...
def _install_windows_launcher() -> Path:
    """Shell links for the app — the ``.lnk`` pair, or the batch fallback.

    The links are the real launcher: they carry two things a ``.cmd`` file
    cannot express at all — an icon, and the :data:`APP_USER_MODEL_ID` that
    lets the taskbar recognize its own window.
    """
    try:
        ...
        _retire_windows_links()  # ours would sit beside the fallback otherwise
        return _install_cmd_file()
...
def _retire_windows_links() -> None:
    """Drop the app's shell links when the batch fallback replaces them.
    ...
    with the identity doing the work the ``@echo off`` sniff does there. Under
    a COM refusal the read fails too and the loop leaves the links alone,
    which is the safe side of the rule.
...
def _install_cmd_file() -> Path:
    """The Windows Desktop batch wrapper — the app's COM-refusal fallback.

    It has no console to report into — ``start`` returns the moment the app is
    handed over, so a launch that fails (a broken install, an import error)
    fails silently, and ``steno --gui`` from a terminal is what shows why.
    That is one of the three costs :func:`_install_windows_links` exists to
    remove, and it is only paid when COM refuses.
    """
    target = _windows_desktop() / "Stenograf.cmd"
    target.parent.mkdir(parents=True, exist_ok=True)
    target.write_text(_WINDOWS_GUI_CMD.format(python=_windowed_python()), encoding="ascii")
    return target
